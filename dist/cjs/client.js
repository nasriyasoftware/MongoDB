...
const filter_1 = __importDefault(require("./assets/filter"));
const query_1 = __importDefault(require("./assets/query"));
const aggregate_1 = __importDefault(require("./assets/aggregate"));
const expressions_1 = __importDefault(require("./assets/expressions"));
const nasriya_uuidx_1 = __importDefault(require("nasriya-uuidx"));
...
class NasriyaDataClient {
    ...
    #_utils = Object.freeze({
        ...
        */
        prepareEvent: (data) => {
            const database = databases_1.default.getDatabase(this.#_config.database.selected);
            if (!database) {
        ...
        checkArgs: async (collectionName, argData, options) => {
            ...
            if (argData.type === 'Object') {
                if (!helpers_1.default.isRealObject(argData.value)) {
        ...
        validateItemSchema: (item, schema, operation) => {
            if (!helpers_1.default.isRealObject(item)) {
            ...
            const checkType = (property, schemaType, value) => {
                switch (schemaType) {
                    ...
                    case 'Object':
                        {
                            if (!helpers_1.default.isRealObject(value)) {
                                throw `The ${property} type is defined in the collection's schema as '${schemaType}', but instead got ${typeof value}`;
                            }
                        }
                        break;
            ...
            if (!helpers_1.default.isUndefined(schema) && helpers_1.default.isRealObject(schema)) {
                for (const property in schema) {
                    const propSchema = schema[property];
                    if (helpers_1.default.isCustomSchema(propSchema)) {
                        if (property in item) {
                            if ('validity' in propSchema && !helpers_1.default.isUndefined(propSchema.validity)) {
    ...
    #_helpers = Object.freeze({
        validateInsertItem: (item, userId) => {
            if ('_id' in item) {
                ...
                item._id = nasriya_uuidx_1.default.v4();
            }
        ...
        checkExistance: async (databaseName, collectionName, itemId) => {
            if (!helpers_1.default.isValidString(itemId)) {
                return false;
            }
        ...
        afterGetItem: async (eventArgs, options) => {
            ...
            if (options?.suppressHooks !== true) {
                if (typeof eventArgs.hook === 'function') {
                    ...
                    if (helpers_1.default.isRealObject(cache.item)) {
                        return cache.item;
                    }
                }
            }
     ...
     * @returns {DataFilter}
     */
    filter() { return new filter_1.default(); }
    /**
    ...
    aggregate(collectionName) {
        ...
        return new aggregate_1.default({ database, collection, authorization: this.authorization, user: this.#_config.user }, this.#_client, this.#_events.onFailure);
    }
    /**
    ...
    query(collectionName) {
        ...
        return new query_1.default({ database, collection, authorization: this.authorization, user: this.#_config.user }, this.#_client, this.#_events.onFailure);
    }
    /**
     ...
     */
    get databases() { return adapter_1.default.databases; }
    /**Create aggregate expressions */
    get aggExps() { return expressions_1.default; }
    /**
    ...
    db(name) {
        ...
        const database = databases_1.default.getDatabase(name, { caseSensitivity: 'ignore' });
        if (!database) {
...
exports.default = NasriyaDataClient;
